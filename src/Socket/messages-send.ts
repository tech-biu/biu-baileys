...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			const content: proto.IMessage | undefined = normalizeMessageContent(message)
			const contentType: keyof proto.IMessage | undefined = getContentType(content)
			if (
				isJidGroup(jid) ||
				(isPnUser(jid) &&
					(contentType === 'interactiveMessage' || contentType === 'listMessage' || contentType === 'buttonsMessage'))
			) {
				const bizNode: BinaryNode = {
					tag: 'biz',
					attrs: {}
				}
				const innerMessage: proto.IMessage =
					message?.viewOnceMessage?.message ??
					message?.viewOnceMessageV2?.message ??
					message?.viewOnceMessageV2Extension?.message ??
					message

				const hasInteractive: boolean = !!innerMessage?.interactiveMessage
				const hasButtons: boolean = !!innerMessage?.buttonsMessage

				if (hasInteractive || hasButtons) {
					bizNode.content = [
						{
							tag: 'interactive',
							attrs: {
