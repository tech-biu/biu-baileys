...
export const generateWAMessageContent = async (
	...
	if ('text' in message) {
		...
		m.orderMessage = proto.Message.OrderMessage.fromObject({
		...
		const stickerMedia = message?.requestPayment?.sticker
		const sticker = stickerMedia ? await prepareWAMessageMedia({ sticker: stickerMedia, ...options }, options) : null
		...
		if (req.sticker) {
			notes = {
				stcikerMessage: {
					...
					contextInfo: req.contextInfo ?? undefined
				}
			}
		} else if (req.note) {
			notes = {
		...
		m.requestPaymentMessage = proto.Message.RequestPaymentMessage.fromObject({
			expiryTimestamp: req.expiry,
			amount1000: req.amount,
			currencyCodeIso4217: req.currency,
			requestForm: req.from,
			noteMessage: { ...notes },
